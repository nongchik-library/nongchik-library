-- Nongchik Library V5
-- ใช้หลังจาก V4 ได้แล้ว สามารถ Run ซ้ำได้
...
-- ป้องกันครูเปลี่ยน created_by และป้องกันครูอนุมัติข้อมูลเอง
...
begin
  if not public.is_admin() then
    ...
    if new.status not in ('pending','rejected') then
      raise exception 'เฉพาะแอดมินเท่านั้นที่อนุมัติข้อมูล';
    end if;
  end if;
  return new;
end;
...
create policy "images insert own folder"
